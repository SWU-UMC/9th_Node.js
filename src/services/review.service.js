...
export const reviewAdd = async (user_id, mission_id, body) => {
  ...
  if (!completedMission) {
    throw new DuplicateUserEmailError("미션을 완료한 사용자만 리뷰를 작성할 수 있습니다.");
  }
  ...
  if(!restaurant) {
    throw new DuplicateUserEmailError ("리뷰를 작성하려는 레스토랑이 존재하지 않습니다.")
  }

  //이 미션에 대해 이미 리뷰를 작성했는지 확인 (중복 방지)
  const existingReview = await findReviewByMission(user_id, mission_id);
  if (existingReview) {
    throw new DuplicateUserEmailError("이미 이 미션에 대한 리뷰를 작성했습니다.");
  }
